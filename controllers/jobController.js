...
export const getAllJobs = async (req, res) => {
    ...
    const sortOptions = {
        newest: '-createdAt', oldest: 'createdAt', 'a-z': 'position', 'z-a': '-position'
    };
...
export const showStats = async (req, res) => {
    ...
    let stats = await Job.aggregate([{$match: {createdBy: new mongoose.Types.ObjectId(req.user.userId)}}, {
        $group: {
    ...
    const defaultStats = {
        pending: stats.pending || 0, interview: stats.interview || 0, rejected: stats.rejected || 0
    };

    let monthlyApplications = await Job.aggregate([{$match: {createdBy: new mongoose.Types.ObjectId(req.user.userId)}}, {
        $group: {
